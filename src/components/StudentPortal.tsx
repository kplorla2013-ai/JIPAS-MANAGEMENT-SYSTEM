...
import { Student, TermReport, StudentBill, PaymentRecord, CalendarEvent, NotificationItem, PaymentSettingsConfig, FeeSubmissionItem } from '../types';
import SchoolCalendarView from './SchoolCalendarView';
import JIPASLogo from './common/JIPASLogo';
import { subscribePaymentSettings, subscribeFeeSubmissions, saveFeeSubmission, saveNotification } from '../services/dbService';
...
import { 
  ...
  Send, Copy, Check, Building2, Smartphone, Plus, HelpCircle, X
} from 'lucide-react';
...
export default function StudentPortal({ student, reports, bills, payments, calendarEvents, notifications = [] }: StudentPortalProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end mb-2">
        <div id="google_translate_element" className="bg-white px-3 py-1 rounded-lg shadow-sm text-xs font-semibold"></div>
      </div>
      {/* Student Top Banner */}
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
              <div>
                <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-emerald-600" /> Latest Academic Performance Summary ({studentReport?.term})
                </h3>
                <p className="text-xs text-slate-500">Official grades registered with West African / GES curriculum standards</p>
              </div>
              <button
                onClick={() => setActiveTab('reports')}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-6">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Academic Year: {studentReport?.academicYear || '2026/2027'} • JIPAS</p>
            </div>
