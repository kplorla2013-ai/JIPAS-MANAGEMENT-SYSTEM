...
import { 
  ...
  PanelLeftClose, PanelLeftOpen, MessageCircle, Database, Trash2, X
} from 'lucide-react';
...
const VALID_ADMIN_MODULES = new Set([
  ...
  // System
  'system_settings', 'system_account_requests', 'account_requests', 'system_backup_restore', 'backup_recovery', 'backup_restore',
...
export default function AdminPortal({
  ...
  // Accordion category collapse/expand state - keeps active category open while minimizing sidebar height
  const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>(() => {
    const initialMod = window.location.hash.replace('#', '') || localStorage.getItem('jipas_active_page_admin') || 'dashboard';
    const cat = getCategoryForModule(initialMod);
    return { [cat]: true };
  });
    ...
    if (currentUser?.role === 'sub_admin' && Array.isArray(currentUser?.allowedModules) && currentUser.allowedModules.length > 0) {
      const allowedItems = group.items.filter(item => {
        return currentUser.allowedModules.some((mod: string) => {
          if (mod === group.id || mod === item.id) return true;
          if (group.id === 'setup' && (mod === 'setup_management' || mod.startsWith('setup'))) return true;
          if (group.id === 'system' && (mod === 'system_settings' || mod.startsWith('system'))) return true;
          if (group.id === 'teacher' && (mod === 'teachers' || mod.startsWith('teacher'))) return true;
          if (group.id === 'student' && (mod === 'students' || mod.startsWith('student'))) return true;
          if (group.id === 'exam' && (mod === 'exams' || mod.startsWith('exam'))) return true;
          if (group.id === 'fee' && (mod === 'fees' || mod.startsWith('fee'))) return true;
          if (group.id === 'notif' && (mod === 'notif_send' || mod.startsWith('notif') || mod.startsWith('sms') || mod.startsWith('whatsapp'))) return true;
          if (group.id === 'logs' && (mod === 'logs_user' || mod.startsWith('logs'))) return true;
          return item.id.startsWith(mod) || mod.startsWith(item.id);
        });
      });
  ...
  useEffect(() => {
    ...
    const cat = getCategoryForModule(activeModule);
    if (cat && cat !== 'dashboard') {
    ...
    <div className="relative flex flex-col lg:flex-row gap-6 w-full">
      ...
      {!isSidebarOpen && (
        <button
          ...
          className="fixed left-0 top-24 z-40 bg-slate-900/95 hover:bg-indigo-600 text-white pl-2.5 pr-3.5 py-2.5 rounded-r-xl shadow-2xl border-y border-r border-slate-700 backdrop-blur-xs transition-all flex items-center gap-2 cursor-pointer group animate-fadeIn"
        >
      ...
      {isSidebarOpen && (
        <aside
          ...
          className="fixed inset-y-0 left-0 z-50 w-72 max-w-[85vw] h-full bg-slate-900 text-slate-300 shadow-2xl flex flex-col border-r border-slate-800 lg:static lg:inset-auto lg:z-30 lg:w-64 xl:w-72 lg:shrink-0 lg:sticky lg:top-24 lg:max-h-[calc(100vh-6.5rem)] lg:rounded-2xl lg:border lg:border-slate-800 overflow-hidden"
          aria-label="Admin Navigation Sidebar"
        >
          {/* Header with static close toggle */}
          <div className="p-3.5 border-b border-slate-800 flex items-center justify-between shrink-0 bg-slate-950/50">
          ...
          <div className="px-3 pt-2.5 pb-1 shrink-0">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-2.5 py-1.5 bg-slate-800/90 border border-slate-700/70 rounded-lg text-[11px] text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-2.5 py-2 space-y-1.5 text-xs font-medium scrollbar-thin scrollbar-thumb-slate-700">
            ...
            {filteredNavGroups.map((group) => {
              ...
              const isExpanded = menuFilter.trim() ? true : Boolean(expandedCategories[group.id]);

              return (
                <div key={group.id} className="border border-slate-800/80 rounded-xl overflow-hidden bg-slate-900/40">
                  <button
                  ...
                  >
                    <span className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider">
                      <GroupIcon className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                  ...
                  {isExpanded && (
                    <div className="p-1 space-y-0.5 bg-slate-950/30 border-t border-slate-800/60">
                      {filteredItems.map(item => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-left transition-all cursor-pointer text-xs ${
                              isSelected
                                ? 'bg-indigo-600 text-white font-bold shadow-xs'
            ...
            <div className="pt-2 border-t border-slate-800/80">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-red-400 hover:bg-red-950/40 hover:text-red-300 transition-colors text-xs font-bold cursor-pointer"
              >
      ...
      <div className="flex-1 min-w-0 w-full space-y-6">
        ...
        {(activeModule.startsWith('setup_') || activeModule === 'academic_setup') && (
        ...
        {activeModule !== 'system_backup_restore' && (activeModule.startsWith('system_') || activeModule === 'system_settings' || activeModule === 'users_roles' || activeModule === 'student_portal_control' || activeModule === 'manage_portal_logins') && (
        ...
        {(activeModule.startsWith('teacher_') || activeModule === 'teachers') && (
          <TeacherManager
            activeModule={activeModule}
            teachers={teachers}
        ...
        {(activeModule.startsWith('fee_') || activeModule === 'fees' || activeModule === 'bills' || activeModule === 'payments' || activeModule === 'income_expenses' || activeModule === 'audit_activity') && (
          <FeeManager
            activeModule={activeModule}
            students={students}
            bills={bills}
        ...
        {(activeModule.startsWith('notif_') || activeModule.startsWith('sms_') || activeModule.startsWith('whatsapp_') || activeModule.startsWith('logs_') || activeModule === 'send_notification' || activeModule === 'notification_history' || activeModule === 'compose_sms' || activeModule === 'sms_history' || activeModule === 'student_login_history' || activeModule === 'user_login_history') && (
