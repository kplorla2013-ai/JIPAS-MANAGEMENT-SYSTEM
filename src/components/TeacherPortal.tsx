...
import { Teacher, Student, TermReport, ClassReportBroadcast, StudentBill, PaymentRecord, CalendarEvent, NotificationItem, TeacherAttendanceRecord } from '../types';
import { 
  ...
  HardDrive, Database, QrCode, Camera, LogIn
} from 'lucide-react';
...
import { saveStudent, saveReport, saveAllReports, subscribeSettings, saveNotification, subscribeTeacherAttendance, saveTeacherAttendanceRecord } from '../services/dbService';
...
export default function TeacherPortal({ 
  ...
  const classesList = Array.isArray(teacher?.classesTaught) ? teacher.classesTaught : [];
  ...
  const subjectsList = Array.isArray(teacher?.subjectsTaught) ? teacher.subjectsTaught : [];
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9] flex flex-col md:flex-row -mx-4 sm:-mx-6 lg:-mx-8 -my-8 font-sans relative">
            ...
            <div className="space-y-1.5">
              ...
              <div className={isSidebarOpen ? "grid grid-cols-2 gap-2 p-1.5 bg-[#17202d] rounded-2xl border border-slate-700/50" : "flex flex-col gap-1.5"}>
                ...
                >
                  ...
                  <span className="text-[10px] font-extrabold text-center leading-tight">
                    {isSidebarOpen ? 'Report Card Broadsheets' : 'Reports'}
                  </span>
                </button>
              </div>
            </div>
        ...
        <div className="bg-white border-b border-slate-200 h-14 px-4 sm:px-6 flex items-center justify-between shadow-2xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
              title="Scan Office QR Code for Attendance"
            >
              <QrCode className="w-4 h-4" />
              <span className="hidden sm:inline">Office QR Attendance</span>
              <span className="sm:hidden">QR Clock-In</span>
            </button>

            <button
              onClick={() => { setQrModalInitialMode('my_qr_badge'); setIsQrModalOpen(true); }}
              className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
              title="My Staff QR Badge"
            >
              <UserCheck className="w-4 h-4" />
              <span className="hidden md:inline">My Badge</span>
            </button>
        ...
        <div className="p-4 sm:px-6 space-y-4">
          ...
          <AnimatePresence mode="wait">
            ...
            <div className="space-y-4">
              ...
              {/* PROMINENT REAL-TIME QR CODE ATTENDANCE & SERVER CLOCK VALIDATION CARD */}
              <div className="bg-gradient-to-r from-slate-950 via-indigo-950 to-slate-900 rounded-2xl p-5 text-white shadow-xl border border-indigo-900/80 space-y-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-3 border-b border-indigo-900/60">
                  <div className="flex items-center gap-3">
                    <div className="p-3 bg-indigo-600/30 rounded-2xl border border-indigo-400/30 shadow-inner shrink-0">
                      <QrCode className="w-7 h-7 text-indigo-300 animate-pulse" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-black text-base sm:text-lg text-white tracking-tight">
                          Office Attendance & Real-Time QR Scanner
                        </h3>
                        <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-extrabold border border-emerald-500/30 tracking-wide uppercase flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
                          Server Clock Validated
                        </span>
                      </div>
                      <p className="text-xs text-indigo-200/80 mt-0.5">
                        Scan the Office QR code or present your Staff Badge upon arrival at the administration office.
                      </p>
                    </div>
                  </div>

                  {/* Live Server Clock Display */}
                  <div className="bg-slate-900/90 border border-indigo-500/30 rounded-xl p-3 text-right shadow-inner min-w-[200px]">
                    <div className="text-[10px] font-mono font-bold text-indigo-300 tracking-wider uppercase flex items-center justify-end gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-amber-400 animate-spin" style={{ animationDuration: '6s' }} />
                      JIPAS Server Time (UTC+0)
                    </div>
                    <div className="text-lg font-black font-mono text-white tracking-wider mt-0.5">
                      {liveServerClock.toLocaleTimeString()}
                    </div>
                    <div className="text-[10px] text-indigo-200 font-medium">
                      {liveServerClock.toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                  {/* Status Box */}
                  <div className="md:col-span-6 bg-slate-900/80 border border-white/10 p-3.5 rounded-xl">
                    <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Today's Attendance Status ({todayStr})</div>
                    
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-950/70 p-2.5 rounded-lg border border-emerald-500/20">
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Sign In (Arrival)</div>
                        {myTodayAttendanceRecord?.timeIn ? (
                          <div className="mt-0.5 text-xs font-mono font-black text-emerald-300 flex items-center gap-1">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                            {myTodayAttendanceRecord.timeIn}
                            <span className="text-[9px] font-sans text-emerald-400/80 uppercase ml-1">({myTodayAttendanceRecord.status})</span>
                          </div>
                        ) : (
                          <div className="mt-0.5 text-xs font-bold text-amber-400 flex items-center gap-1">
                      ...
                      <div className="bg-slate-950/70 p-2.5 rounded-lg border border-blue-500/20">
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Sign Out (Departure)</div>
                        {myTodayAttendanceRecord?.timeOut ? (
                          <div className="mt-0.5 text-xs font-mono font-black text-blue-300 flex items-center gap-1">
                            <CheckCircle2 className="w-3.5 h-3.5 text-blue-400" />
                            {myTodayAttendanceRecord.timeOut}
                          </div>
                        ) : (
                          <div className="mt-0.5 text-xs font-medium text-slate-400">
                            {myTodayAttendanceRecord?.timeIn ? 'Signed In — Not Signed Out Yet' : 'Not Signed Out'}
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Actions */}
                  <div className="md:col-span-6 flex flex-wrap sm:flex-nowrap items-center gap-2.5">
                    <button
                      onClick={() => { setQrModalInitialMode('scan_office_qr'); setIsQrModalOpen(true); }}
                    ...
                    <button
                      onClick={() => { setQrModalInitialMode('my_qr_badge'); setIsQrModalOpen(true); }}
                      className="px-4 py-3 bg-indigo-600/80 hover:bg-indigo-600 text-white rounded-xl text-xs font-bold border border-indigo-400/30 flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md"
                    >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 group-hover:text-rose-100 transition-colors mt-0.5 line-clamp-2">
                        Change portal passcode & auth settings
                      </p>
                    </div>
                  </button>
                </div>
      ...
      {/* Teacher QR Code Office Attendance Modal */}
      <TeacherQrAttendanceModal
        isOpen={isQrModalOpen}
        onClose={() => setIsQrModalOpen(false)}
        currentTeacher={teacher}
        teachersList={teachers.length > 0 ? teachers : [teacher]}
        attendanceRecords={attendanceRecords}
        initialMode={qrModalInitialMode}
        hideOfficeTerminal={true}
      />
    </div>
  );
}
