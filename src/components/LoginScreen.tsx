...
export default function LoginScreen({ onLogin, studentsList, teachersList = [] }: LoginScreenProps) {
  ...
  const handleDemoLogin = (role: 'teacher' | 'accountant' | 'student') => {
    ...
    if (role === 'teacher') {
      ...
      demoUser = {
        ...
        name: firstTeacher?.name || 'Ebenezer Frimpong',
        email: firstTeacher?.email || 'teacher@jipas.edu.gh',
        role: 'teacher',
        classAssigned: firstTeacher?.classAssigned || 'Basic 1',
        avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80'
      };
      setAuthSuccessNotice(`⚡ Free Demo Access: Logged in as Teacher (${demoUser.name})!`);
      ...
      demoUser = {
        id: firstStudent?.id || 'std-demo-1',
        name: firstStudent?.name || 'Kwame Nkrumah',
        email: firstStudent?.email || 'adm260001@student.jipas.com',
        role: 'student',
        admissionNo: firstStudent?.admissionNo || 'ADM/26/0001',
        avatar: 'https://images.unsplash.com/photo-1543269865-cbf427effbad?w=150&auto=format&fit=crop&q=80'
      };
      setAuthSuccessNotice(`⚡ Free Demo Access: Logged in as Student (${demoUser.name})!`);
    }
  ...
  const executeStudentRegistration = async (emailVerified?: string) => {
    ...
    try {
      ...
      const cleanAdmNo = studentAdmissionNo.trim().toUpperCase();
      ...
      setRegSuccessNotice(`Verification successful! Student registration for ${studentFullName} (${cleanAdmNo}) has been submitted and is pending Administrator approval.`);
      ...
      setTimeout(() => {
        setViewMode('login');
        setAuthSuccessNotice('Student registration verified & submitted! You can sign in once approved by Admin.');
      }, 3500);
    } catch (err: any) {
  ...
  const handleStudentRegister = async (e: React.FormEvent) => {
    ...
    // Compulsory Student Administration Number check
    if (!cleanAdmNo) {
      setRegErrorNotice('Student Administration Number is compulsory for student identity verification. Please enter your official Student Admission / Index Number.');
      return;
    }

    // Check 1: Prevent duplicate account creation for the same administration number
    const storedUsers = getStoredUsers();
    const isAlreadyRegistered = storedUsers.some(u => 
      u.admissionNo && u.admissionNo.toLowerCase().trim() === cleanAdmNo.toLowerCase()
    );
    if (isAlreadyRegistered) {
      setRegErrorNotice(`An account with Student Administration Number '${cleanAdmNo}' is already registered. Please sign in using your credentials or contact administration.`);
      return;
    }

    // Check 2: Verify Student Administration Number against school's enrolled students list
    if (studentsList && studentsList.length > 0) {
      const matchedStudent = studentsList.find(s => 
        s.admissionNo && s.admissionNo.toLowerCase().trim() === cleanAdmNo.toLowerCase()
      );

      if (!matchedStudent) {
        setRegErrorNotice(`Verification Failed: Student Administration Number '${cleanAdmNo}' was not found in the official school enrollment database. Please verify your admission document or contact JIPAS Administration.`);
        return;
      }
    }
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center py-10 px-4 sm:px-6 relative overflow-hidden">
      ...
      {viewMode === 'login' && (
        <div className="w-full max-w-[440px] bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 p-7 sm:p-8 z-10 transition-all duration-300">
          ...
          {/* Quick Demo Free Login Section */}
          <div className="mt-5 p-3.5 bg-gradient-to-br from-indigo-50/90 via-blue-50/60 to-emerald-50/90 rounded-2xl border border-indigo-200/80 shadow-xs space-y-2.5">
            <div className="flex items-center justify-between">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className="text-[10px] font-extrabold uppercase text-blue-600 group-hover:text-blue-300">Teacher</span>
                  <Briefcase className="w-3.5 h-3.5 text-blue-500" />
                </div>
                <div className="text-xs font-bold text-slate-800 group-hover:text-white truncate">Class Teacher</div>
                <div className="text-[9px] text-slate-500 group-hover:text-slate-300">Scores & Broadsheet</div>
              </button>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className="text-[10px] font-extrabold uppercase text-emerald-600 group-hover:text-emerald-300">Bursar</span>
      ...
      {viewMode === 'register_student' && (
        <div className="w-full max-w-[480px] bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 p-7 sm:p-8 z-10 animate-fade-in">
          ...
          <form onSubmit={handleStudentRegister} className="space-y-3.5 text-xs">
            {/* Student Verification Identity Box */}
            <div className="p-3 bg-emerald-50/80 border border-emerald-200/90 rounded-2xl space-y-1.5 text-xs text-emerald-950">
              <div className="flex items-center gap-2 font-black text-emerald-900">
                <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
                <span>Student Identity Verification Required</span>
              </div>
              <p className="text-[11px] text-emerald-800 leading-snug font-medium">
                Your Student Administration Number (Admission No / Index No) is compulsory for account creation. It verifies your active enrollment in the JIPAS database.
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] font-black text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded-full border border-emerald-200 uppercase tracking-wider">
              ...
              <input
                ...
                placeholder="e.g. ADM/26/0001 or 2026-0012"
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-emerald-300 rounded-xl font-mono font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-emerald-500 uppercase tracking-wider"
              />
            </div>
