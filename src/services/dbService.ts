...
export { getStoredClassFeeTariffs, saveStoredClassFeeTariffs, INITIAL_CLASS_FEE_TARIFFS };
...
export function generateNewStaffSecretCode(adminName: string = 'System Admin'): StaffSecretCodeRecord {
  ...
  saveStoredStaffSecretCode(newRecord.code);
  return newRecord;
}
...
export function validateAndConsumeStaffSecretCode(codeStr: string, userEmail: string): { success: boolean; error?: string } {
  ...
  // Allow default fallback code
  if (cleanCode === DEFAULT_STAFF_SECRET_CODE) {
    return { success: true };
  }

  const codes = getStoredStaffSecretCodes();
  const found = codes.find(c => c.code.toUpperCase() === cleanCode.toUpperCase());

  if (!found) {
    return { success: false, error: 'Invalid Staff Secret Code. Please check with your School Administrator.' };
  }

  if (found.used) {
    return { success: false, error: `This secret code has already been used by ${found.usedBy || 'another user'}. Each code is strictly one-use per user.` };
  }

  const now = Date.now();
  if (now > found.expiresAt) {
    return { success: false, error: 'This secret code has expired (valid for only 10 minutes). Please request a fresh code from Admin.' };
  }
...
export async function saveStaffSecretCode(code: string): Promise<void> {
  saveStoredStaffSecretCode(code);
  await saveSettings({ staffSecretCode: code });
}
