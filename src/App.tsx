        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2">
                <p className="text-xs font-semibold text-indigo-600 truncate">
                  {t('app.subtitle', 'School Management System • Est. 1990')}
                </p>
                {dbSynced && (
                  <span className="hidden lg:inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.2 rounded-full whitespace-nowrap">
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-700">
              {currentUser.role === 'admin' && <ShieldCheck className="w-4 h-4 text-red-600" />}
              {currentUser.role === 'sub_admin' && <Shield className="w-4 h-4 text-amber-600" />}
              {currentUser.role === 'teacher' && <UserCheck className="w-4 h-4 text-emerald-600" />}
              {currentUser.role === 'accountant' && <Calculator className="w-4 h-4 text-cyan-600" />}
              {currentUser.role === 'student' && <BookOpen className="w-4 h-4 text-indigo-600" />}
              {currentUser.role === 'clerk' && <UserCog className="w-4 h-4 text-purple-600" />}
              <span className="capitalize">{t('app.role', 'Role')}: {t(`app.role.${currentUser.role}`, currentUser.role)}</span>
            </div>
